...
/*fg命令*/          
void fg_exec(int pid){   //////@@@@@@@@@@@@@@@@@@@@@@@@@@@修改点   
    Job *now = NULL; 
	int i;
    sigset_t sigs;//定义信号集
    sigemptyset(&sigs);///空信号集
    sigaddset(&sigs,SIGSTOP);///屏蔽ctrl_Z发出的信号
    //SIGCHLD信号产生自此函数
    ingnore = 1;
...
void exeWildcard(char *args)
{
	int i, pos, slash1, slash2, len, start, end, n, flg;
	char *path, *file_name, *tmp, *buff_end, *buff_start;
	DIR *d;
	struct dirent *dirent;

	for (pos = posWildcard(args); pos >= 0; pos = posWildcard(args)){
		slash1 = slash2 = -1;
		len = strlen(args);
		...
		if (d){
			...
			while (dirent = readdir(d)){
				...
				if (matchStr(tmp, file_name, 1)){
					...
					args = (char *)malloc(sizeof(char)*n);
					strcpy(args, buff_start);
					strcat(args, file_name);
					strcat(args, buff_end);
					len = strlen(args);
					flg = 1;
					break;
				}
...
int do_simple_cmd(int argc, char** argv, int prefd[], int postfd[],int back)
{
	...
	if((pid = fork())<0)
	{
		...
		if(prefd) {//has a pre pipe, redirect stdin；在没有输入重定向的情况下从前管道中读数
